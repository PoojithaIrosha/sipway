...
if (empty($fname)) {
    ...
    $studentRs = MySQL::search("SELECT * FROM student WHERE email = '${email}' OR mobile = '${mobile}'");
    if ($studentRs->num_rows > 0) {
        ...
        $trial_period = date("Y-m-d", strtotime("+1 month", strtotime($creation_date)));

        $verification_code = substr(md5(uniqid(rand(), true)), 16, 6);
        $password = substr(md5(uniqid(rand(), true)), 16, 8);

        // Register new student
        MySQL::iud("INSERT INTO student(email, first_name, last_name, mobile, password, batch_id,verification_code, creation_time, trial_period) VALUE ('${email}','${fname}', '${lname}', '${mobile}', '${password}', '${batch}','${verification_code}', '${creation_date}', '${trial_period}')");

        // Send email verification to the newly created student
        ...
        try {
            ...
            $mail->Username = 'suwanisandunika0723@gmail.com'; // SMTP username TODO: Change the email
            $mail->Password = 'asahbizcfgkwgmty'; // SMTP password // TODO: Change the password
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS; // Enable implicit TLS encryption
            $mail->Port = 465; // TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

            // Recipients
            $mail->setFrom('suwanisandunika0723@gmail.com', 'Sipway'); // TODO: Change the email
            $mail->addAddress($email, $fname . ' ' . $lname); // Add a recipient
            $mail->addReplyTo('suwanisandunika0723@gmail.com', 'Sipway'); // TODO: Change the email

            // Content
            $mail->isHTML(true); // Set email format to HTML
            $mail->Subject = 'Verify Student Account - SipWay';
                                    ...
                                    <table border="0" cellpadding="0" cellspacing="0" class="image_block block-2"
                                        ...
                                        <tr>
											<div style="display: flex; justify-content: center;">
												<span style="font-size:30px;color:#2b303a;font-family: Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif; "><strong>SipWay - Student Account</strong></span>
											</div>
                                        </tr>
                                    </table>
                                            ...
                                            <td class="pad"
                                                ...
                                                <div align="center" class="alignment">
                                                    ...
                                                    <div style="text-decoration:none;display:inline-block;color:#ffffff;background-color:#ff0070;border-radius:60px;width:auto;border-top:0px solid transparent;font-weight:400;border-right:0px solid transparent;border-bottom:0px solid transparent;border-left:0px solid transparent;padding-top:15px;padding-bottom:15px;font-family:Montserrat, Trebuchet MS, Lucida Grande, Lucida Sans Unicode, Lucida Sans, Tahoma, sans-serif;font-size:16px;text-align:center;mso-border-alt:none;word-break:keep-all;">
                                                        ...
                                                        <a href="http://localhost/sipway/student/verify-student.php?student_code=' . $verification_code . '" style="margin: 0; word-break: break-word; line-height: 32px; color: white; text-decoration: none;"><strong>Verify Your Email</strong></a>
                                                        </span>
                                                    </div>
													<div style="margin-top: 25px">
														<span style="font-size:15px;font-family: Tahoma; color:gray;">Please click the button verify email to verify your newly created student account</span>
													</div>
